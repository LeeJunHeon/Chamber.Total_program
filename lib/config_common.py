...
# ======================================================================
OES_AVG_COUNT = 3
OES_PLOT_MIN_INTERVAL_S = 0.5   # OES 그래프 최소 갱신 간격(초). 이내 연속 수신은 건너뛴다(루프 부하 방지)
...
# --- DC Pulse: Operation Mode / Arc 알림 (EnerPulse EP5 매뉴얼 품기28-EPPEP102-V08) ---
...
# ----------------------------------------------------------------------
# 전압 측정: 장비(0x9A READ_PIV)가 이미 V 단위 정수로 준다 — 2026-09-21 실측 1,624표본에서
# P = V_raw x I 가 성립. 구 1.468815 는 근거 불명(전압이 약 47% 부풀려져 기록됐었음).
DCP_V_MEAS_V_PER_LSB = 1.0
...
# ======================================================================
#  개발자 모드 (로컬 개발 PC 전용)
#  - True: 챗 웹훅 / 모니터 웹훅 / 구글드라이브 시트 기록 / 장치 자동연결 차단
#  - 장비 PC의 settings.json에서는 반드시 false 유지
# ======================================================================
DEV_MODE = False
...
CHAT_PENDING_MAX = 200         # 대기 전송 태스크 상한(초과 카드는 드롭, 다음 카드에 건수 표기). urgent 카드는 통과
CHAT_PENDING_HARD_MAX = 1000   # urgent(공정 실패·인터락) 카드도 넘지 못하는 하드 상한
...
PLC_COIL_LOG_BACKOFF_AFTER = 2  # 저우선 연속 실패 이 횟수부터 로거가 스스로 쉰다 (0=사용 안 함)
...
CHAMBER_RF_CONT_RAMPDOWN_INTERVAL_MS = 1000
...
# Main Valve 시작 인터락: 공정 시작 전 Main Valve(M_V) 열림 확인 (M_V_SW AND M_V_인터락)
CHAMBER_MAINVALVE_RECHECK_COUNT = 5         # 열림 재확인 최대 횟수
CHAMBER_MAINVALVE_READ_TIMEOUT_S = 0.6      # 코일 1개 읽기 타임아웃 (초)
CHAMBER_MAINVALVE_RECHECK_INTERVAL_S = 0.2  # 재확인 간격 (초)

CHAMBER_CHUCK_MOVE_TIMEOUT_S = 120.0
...
# ======================================================================
# 공정 모니터링 (Main Shutter open 후 gas/pressure 편차 알림)
# ======================================================================
...
RF_READER_NAS_DIR           = r"\\VanaM_NAS\VanaM_Sputter\Sputter\Logs\CH1&2\Camera_Logs"  # CSV 사본 전송 대상
