...
class PlasmaCleaningRuntime:
    ...
    - IG/건셔터/가스밸브(PLC) 제어 없음(가스밸브 항상 open 전제)
    - MFC(CH1)만으로 유량 제어
    - RF 파워는 '연속'만 사용하며, 반드시 PLC를 통해 적용/읽기/끄기
    - RF 펄스는 사용하지 않음
    """
    ...
    def __init__(
        ...
        log_dir: Path,
        *,
        plc: Any,                              # ★ PLC 공유 인스턴스 (필수)
        chat: Optional[Any] = None,
        # 콜백 방식이 필요하면 유지하지만, 기본은 PLC 사용
        rf_apply: Optional[Callable[[float], Coroutine[Any, Any, None]]] = None,
        rf_read: Optional[Callable[[], Coroutine[Any, Any, Mapping[str, float]]]] = None,
        rf_off: Optional[Callable[[], Coroutine[Any, Any, None]]] = None,
    ) -> None:
        self.ui = ui
        ...
        # (선택) 직접 콜백 — 기본은 PLC 사용. 콜백이 주어지면 우선 사용.
        self._rf_apply_cb = rf_apply
        self._rf_read_cb = rf_read
        self._rf_off_cb = rf_off
    ...
    # ─────────────────────────────────────────────────────────────
    # 컨트롤러 바인딩 (MFC + RF 연속만 / RF는 PLC 경유)
    # ─────────────────────────────────────────────────────────────
    def _bind_plasma_controller(self) -> None:
        ...
        def cb_rf_power(value: float) -> None:
            async def run():
                try:
                    if callable(self._rf_apply_cb):
                        # 외부 콜백이 있으면 그것을 사용
                        await self._rf_apply_cb(float(value))
                    else:
                        # ★ PLC로 RF 연속 파워 적용 (CH2에서 쓰던 패턴 그대로)
                        # family="DCV", channel=1은 RF 채널로 사용하는 기존 규약
                        await self.plc.power_apply(float(value), family="DCV", ensure_set=True, channel=1)

                    # 상태 읽기(가능 시)
                    fwd = ref = None
                    if callable(self._rf_read_cb):
                        st = await self._rf_read_cb()
                        fwd = float(st.get("forward", 0.0))
                        ref = float(st.get("reflected", 0.0))
                    else:
                        with contextlib.suppress(Exception):
                            fwd_raw = await self.plc.read_reg_name("DCV_READ_2")
                            ref_raw = await self.plc.read_reg_name("DCV_READ_3")
                            fwd = float(fwd_raw); ref = float(ref_raw)

                    if fwd is not None and ref is not None:
                        with contextlib.suppress(Exception):
                            self.data_logger.log_rf_power(fwd, ref)
                        self._set("forP_edit", f"{fwd:.2f}")
                        self._set("refP_edit", f"{ref:.2f}")

                    self.plasma.on_rf_ok()
                except Exception as e:
                    self.append_log("RF", f"RF 파워 적용 실패: {e!r}")
                    self.plasma.on_rf_failed(str(e))
            self._spawn_detached(run())

        def cb_rf_off() -> None:
            async def run():
                try:
                    if callable(self._rf_off_cb):
                        await self._rf_off_cb()
                    else:
                        # ★ PLC로 RF 출력 0 W
                        await self.plc.power_write(0.0, family="DCV", write_idx=1)
                except Exception as e:
                    self.append_log("RF", f"RF OFF 예외: {e!r}")
                finally:
                    self.plasma.on_rf_off_ok()
            self._spawn_detached(run())
    ...
    def _handle_start_clicked(self, _checked: bool = False):
        ...
        def _flow(leaf: str) -> float:
            t = self._get_text(leaf) or "0"
            v = float(t)
        ...
        try:
            rf_txt = self._get_text("rfPower_edit") or self._get_text("dcPower_edit")
            rf_power = float(rf_txt or "0")
            if rf_power <= 0: raise ValueError()
        except Exception:
            self._post_warning("입력값 확인", "RF Power(W)를 확인하세요.")
            return

        try:
            t_proc = float(self._get_text("processTime_edit") or "0")
            if t_proc <= 0: raise ValueError()
        except Exception:
            self._post_warning("입력값 확인", "Process Time(초)을 확인하세요.")
            return

        params = {
            "process_note": "PlasmaCleaning",
            "ar_flow": ar_flow, "o2_flow": o2_flow,
            "rf_power": rf_power, "process_time": t_proc,
        }
        ...
        # maybe_start: 이미 켜져 있으면 내부에서 skip
        self._spawn_detached(self.mfc.start())

        try:
            self.plasma.start_cleaning(params)
        except Exception as e:
