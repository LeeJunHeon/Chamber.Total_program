...
class HostHandlers:
    ...
    async def ms_open(self, data: dict, ch: int | None = None) -> dict:
        return await self._main_shutter_set(data, ch=ch, open_=True)

    async def ms_close(self, data: dict, ch: int | None = None) -> dict:
        return await self._main_shutter_set(data, ch=ch, open_=False)

    def _resolve_ch(self, data: dict, ch: int | None) -> int:
        if ch is None:
            ch = int(data.get("ch", 0))
        if ch not in (1, 2):
            raise ValueError(f"invalid ch={ch} (expected 1 or 2)")
        return ch

    async def _main_shutter_set(self, data: dict, ch: int | None, open_: bool) -> dict:
        ch = self._resolve_ch(data, ch)

        action = "MS_OPEN" if open_ else "MS_CLOSE"
        tag = f"CH{ch}_{action}"

        # ✅ 1) 공정 중이면 막기 (gate_open 패턴과 동일)
        busy_resp = self._fail_if_ch_busy(ch, tag)
        if busy_resp:
            return busy_resp

        interlock_key = f"MAIN_SHUTTER_{ch}_인터락"

        lock = self.ctx.lock_ch1 if ch == 1 else self.ctx.lock_ch2

        async with lock:
            async with self._plc_command(tag):
                self._log_client_request(data)

                # ✅ 2) 인터락 확인
                async with self._plc_call():
                    ok = await self.ctx.plc.read_bit(interlock_key)

                if not ok:
                    # 여기 error_code는 프로젝트 스타일에 맞게 원하는 번호로 통일해도 됨
                    return self._fail(
                        f"{interlock_key}=FALSE → {tag} 불가",
                        code="E331",
                    )

                # ✅ 3) Main Shutter open/close write 후 종료
                # plc.main_shutter는 MAIN_SHUTTER_{ch}_SW에 bool(open_)를 write하는 래치 동작임
                async with self._plc_call():
                    await self.ctx.plc.main_shutter(ch, open=open_)

                return self._ok(f"{tag} 완료")
